...
## ☑ External validation — POC findings vs the 2023–2026 literature (2026-06-30)

After the multi-run medians, we ran a fact-checked deep-research pass (26 sources, 123 extracted claims, 25 adversarially verified by 3-vote: 15 confirmed / 10 refuted) to test our four POC claims against external industry practice and the research literature. **Net: the literature confirms and sharpens the POC's qualitative thesis — graph context is a capability/reliability win on structural queries, not a general token-saver — while leaving the enterprise-scale question open exactly where we left it.**

| # | POC claim | external verdict | why |
|---|---|---|---|
| 1 | On-disk code → agents default to grep, leave a loaded graph unused; no token savings on everyday in-repo localize-and-fix | **behavior SUPPORTED; token-savings half UNVERIFIED** | Claude Code/Windsurf/Cursor default to ripgrep; agents almost never invoke graph/semantic tools organically (Sourcegraph CodeScaleBench: 8 graph "Deep Search" calls across 602 MCP runs vs 4,813 keyword calls). But no external benchmark cleanly isolates *token cost* on local localize-and-fix; the one paper claiming ~10× graph token savings was refuted in our verification. |
| 2 | The graph's real value is capability/completeness — cross-file/transitive and impact/blast-radius reasoning — **not** general token reduction | **SUPPORTED (best-supported claim)** | Four independent primary papers converge: LocAgent (up to 92.7% file localization via multi-hop), RANGER (beats embeddings on RepoBench cross-file retrieval), StackRepoQA (Neo4j structural graph 6.12 vs 5.66 dense-RAG, p<0.0002, "majority of gains"), GraphRAG-Bench (graphs win on multi-hop, tie/lose on single-hop). Consistent boundary = our "graph wins structural, grep wins simple." |
| 3 | Agents won't use graph/MCP tools unless the task genuinely requires them **or** they are explicitly steered | **SUPPORTED** | Same CodeScaleBench behavior, plus "Is Grep All You Need?" — swapping the *harness* (system prompt, tool descriptions, how hits are rendered) shifts accuracy as much as swapping the retriever. Steering matters as much as the tool itself. |
| 4 | At ~100 repos / billions of LOC, graphs help most because grepping everything is infeasible — but unproven on our 2-repo harness | **UNVERIFIED** | Every confirmed graph-value source is *single-repo*. The two scale/cross-repo supports (Sourcegraph Kubernetes "grep 2h timeout vs 89s"; cross-repo precision gains) were both refuted — single-vendor, unreproduced. External evidence neither confirms nor contradicts; genuinely open, mirroring our own admission. |

**Sharper points the literature adds (beyond confirming our four claims):**
- **"Just use a bigger context window" is not the alternative to a graph.** Dumping a whole repo into long context degrades performance (LongCodeBench: 29%→3% as context grows; "lost in the middle"/context-rot). At scale neither grep-everything nor read-everything works — which *motivates* structured retrieval but does not by itself prove graphs are the answer (StackRepoQA, LoCoBench).
- **Graph build/retrieval cost scales badly.** ~91s to build a graph index on ~100K-LOC, and 40–97s per retrieval on 500K+-LOC repos, vs ripgrep ~1.45s (GrepRAG). The cost wall is real and bears directly on whether a graph survives to enterprise scale.
- **Grep and graph are complementary, not substitutes.** GrepRAG reports only ~20–30% solution overlap between lexical and structural methods → a hybrid router (grep for single-hop, graph for multi-hop/cross-repo) is the likely right design, consistent with our "adopt selectively" recommendation.

**Caveats on this external pass:** several load-bearing numbers come from non-peer-reviewed arXiv preprints by interested parties (GrepRAG authors advocate grep; RANGER self-reports its own benchmark), so they carry pro-method framing bias. The strongest harness-steering evidence ("Is Grep All You Need?") is on conversational-memory QA, supporting claim 3 by mechanism rather than direct code-domain measurement. Token cost is the weakest-evidenced question on both sides. Benchmarks are heterogeneous (Acc@5 vs exact-match vs 1–10 judge score; Python vs Java), so cross-paper numbers are directional, not directly comparable. The field moves monthly — Claude Code added native LSP in v2.0.74 (Dec 2025) yet grep still dominated as of early 2026 — so claims 1 and 3 are true *now*, not durable laws.

**Sources (verified):** LocAgent — [ACL 2025](https://aclanthology.org/2025.acl-long.426/) / [arXiv 2503.09089](https://arxiv.org/pdf/2503.09089); RANGER — [arXiv 2509.25257](https://arxiv.org/pdf/2509.25257); StackRepoQA — [arXiv 2603.26567](https://arxiv.org/pdf/2603.26567); GraphRAG-Bench — [arXiv 2506.05690](https://arxiv.org/html/2506.05690v3); GrepRAG — [arXiv 2601.23254](https://arxiv.org/pdf/2601.23254); "Is Grep All You Need?" — [arXiv 2605.15184](https://arxiv.org/html/2605.15184v1); Sourcegraph CodeScaleBench — [blog](https://sourcegraph.com/blog/codescalebench-testing-coding-agents-on-large-codebases-and-multi-repo-software-engineering-tasks). **Refuted in verification** (do not cite as support): the ~10× token-savings claim ([arXiv 2603.27277](https://arxiv.org/html/2603.27277v1)) and the Sourcegraph large-scale/cross-repo gains.
