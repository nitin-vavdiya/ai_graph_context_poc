...
1. **Snapshot-restore** the repo to a pristine baseline — `git stash create` captures tracked state; restored with `git restore --source=<snap>`; only files created during the cell are removed. **Preserves pre-existing local state** (a modified `AGENTS.md`, untracked `documents-latest/`, `.serena/`) — never `reset --hard`.
