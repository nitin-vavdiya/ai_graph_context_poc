...
- **Re-indexing a repo WIPES its `CALLS_SERVICE` edges** — those edges are injected from the C4 model by `poc/enrich/enrich.py`, not derived from code, so `codegraphcontext index <repo> --force` drops the ones touching that repo. **After any re-index, re-run** `NEO4J_PASSWORD=poctestpassword uv run --with neo4j python poc/enrich/enrich.py` (idempotent) and verify `MATCH ()-[r:CALLS_SERVICE]->() RETURN count(r)` = 4.
